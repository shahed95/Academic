...
int32_t main()
{
    ...
    while(t--){
        ...
        vll b(1<<16,0),pre(1<<16,0);
        ll ans=0;
        for(int st=1;st<=n;st+=bl){
            int en=min(n,st+bl-1);
            vll cnt(1<<16,0);
            vll nw(1<<16,0);
            for(int i=st;i<=en;i++){
                for(int j=i+1;j<=en;j++){
                    if(a[j]>a[i]) ans+=pre[a[j]-a[i]]+cnt[a[j]-a[i]];
                    if(a[i]+a[j]<(1<<16)) nw[a[i]+a[j]]++;
                }
                cnt[a[i]]++;
                ans+=b[a[i]];
            }
            vll tmp=multiply(pre,cnt);
            b.resize(1<<16);
            for(int i=0;i<(1<<16);i++) b[i]+=nw[i]+tmp[i],pre[i]+=cnt[i];
        }
        pf1ll(ans);
    }
    return 0;
